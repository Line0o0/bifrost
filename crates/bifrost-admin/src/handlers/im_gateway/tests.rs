    ...
    if cfg!(windows) {
        (
            ...
            vec![
                ...
                "-NonInteractive".to_string(),
                "-Command".to_string(),
                "$null=[Console]::In.ReadToEnd(); $q=[char]34; $content=if ($env:MODEL_OVERRIDE -eq 'gpt-schedule' -and $env:BASE_ENV -eq 'runner') { 'OVERRIDE_OK' } else { 'OVERRIDE_MISSING' }; [Console]::Out.WriteLine('{' + $q + 'type' + $q + ':' + $q + 'assistant_final' + $q + ',' + $q + 'content' + $q + ':' + $q + $content + $q + '}')".to_string(),
            ],
        )
    } else {
        (
